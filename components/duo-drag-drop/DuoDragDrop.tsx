...
export interface DuoDragDropProps {
  ...
  renderWord?: (word: string, index: number) => React.JSX.Element;
  /** Overrides the default Lines renderer */
  renderLines?: (props: { numLines: number; containerHeight: number; lineHeight: number }) => React.JSX.Element;
...
function ComputeWordLayout({
  ...
  return (
    <View
      className={cn(
        "flex-row flex-wrap opacity-0",
        { "justify-center": wordBankAlignment === "center" },
        { "justify-start": wordBankAlignment === "left" },
        { "justify-end": wordBankAlignment === "right" }
      )}
      onLayout={(e) => {
